...
class BinarySystem:
    ...
    def shift_to_com_frame(self):
        """Shift both bodies so the center of mass is at the origin with zero velocity.
        Called after every mass transfer event to maintain the COM frame invariant."""
        R, V = self._com()
    ...
    def compute_energy(self) -> float:
        """Compute total mechanical energy in the current frame.
        Because shift_to_com_frame() is called after every transfer event, the
        COM velocity is always zero and this equals the energy in the COM frame."""
    ...
    def _decompose_perp(v: np.ndarray, r: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        """Decompose velocity v into radial (along r) and tangential (perpendicular) components."""
        rn = np.linalg.norm(r)
        if rn < 1e-30:
    ...
    def _enforce_L_perp(self, L_target: float):
        """Rescale tangential velocity components uniformly to restore angular momentum
        to L_target (Section 3.5, eq. rescale). Radial components are left unchanged.
        Operates in the COM frame; shift_to_com_frame() must be called beforehand."""
    ...
    def transfer_mass(self, dm: float, enforce_angular_momentum: bool = True, L_target: float | None = None):
        """Apply one instantaneous relocation mapping step (Section 3.2):
        remove dm from donor, deposit on accretor at accretor's velocity,
        then optionally apply the angular momentum closure condition (Section 3.5).
        L_target should be L0, the pre-transfer angular momentum, for conservative transfer."""
        if dm <= 0:
            return
        self.donor.mass -= dm
...
def analytical_separation_ratio(M1_i: float, M2_i: float, M1_f: float, M2_f: float) -> float:
    """Return the final-to-initial semi-major axis ratio a_f/a_0 predicted by
    conservative mass transfer (Equation eq:afinal in the paper).
    Returns a dimensionless ratio, not a physical separation."""
...
def run_simulation(
    ...
    n_orbits: float = 50,
    steps_per_orbit: int = 10000,   # dt = 1e-4 P_orb, matching Table 1 of the paper
    ...
    for step in range(n_steps):
        ...
        if transfer_count < n_transfer_steps:
            ...
            while transfer_count < target and transfer_count < n_transfer_steps:
                # L0 is passed as L_target: conservative transfer preserves the
                # initial angular momentum at every step (Section 3.5).
    ...
    a_ana = analytical_separation_ratio(M1, M2, M1f, M2f)
...
def generate_all_results(output_dir: str = ".", use_cache: bool = True):
    ...
    if not enforced:
        # n_steps = n_orbits * steps_per_orbit; used here only for progress bar total.
        # steps_per_orbit=10000 matches Table 1 (dt = 1e-4 P_orb).
        n_steps_progress = 50 * 10000
        prog = _mk_progress()
        if prog is None:
            ...
            with prog:
                for q0, name in zip(mass_ratios, case_names):
                    tE = prog.add_task(f"Case {name} (q0={q0}) | enforced", total=n_steps_progress)
                    rE = run_simulation(q0, enforce_angular_momentum=True, progress=prog, task_id=tE)
                    prog.update(tE, completed=n_steps_progress)

                    tN = prog.add_task(f"Case {name} (q0={q0}) | non-enforced", total=n_steps_progress)
                    rN = run_simulation(q0, enforce_angular_momentum=False, progress=prog, task_id=tN)
                    prog.update(tN, completed=n_steps_progress)
